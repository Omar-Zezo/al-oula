...
import ImagesContainer from "../Media center/ImagesContainer";
import { t } from "i18next";
import UseLangDetection from "../../hooks/UseLangDetection";

const MediaCenterPhotos = () => {
        ...
        if (data.data.data) {
          if (data.data.data.photos) {
            if (data.data.data.photos.data) {
              setSectionList(data.data.data.photos.data);
              setSelectedSectionId(data.data.data.photos.data[0]?.id);
            }
          }
        }
      }
      ...
      <div className="flex flex-col">
        <div className="flex flex-col items-center">
          <FontAwesomeIcon
            ...
            icon={faQuoteRight}
          />
          <h2 className="text-5xl text-mainColor text-center font-bold">{t('photos')}</h2>
        </div>
          ...
          <ul className={`container flex overflow-x-auto max-xl:pb-5 items-center gap-5`}>
            {sectionsList?.map((section) => (
              ...
              >
                ...
                >
                  {section?.photos_count}
                </p>
              </li>
            ))}
          </ul>
          <ImagesContainer selectedSectionId={selectedSectionId} langDetection={langDetection}/>
        </div>
      </div>
    </div>
  );
...
export default MediaCenterPhotos;
